...
//   // Initialize Mapbox map with bounds to restrict panning
//   map.value = new mapboxgl.Map({
//     container: mapContainer.value!,
//     style: 'mapbox://styles/mapbox/streets-v11',
//     center: [3.3792, 6.5244], // Lagos, Nigeria coordinates
//     zoom: 12,
//     // maxBounds: mapBounds, // Constrain the map within the Lagos bounding box
//   });
...
//   // If an existing location is already present, add a marker on the map
//   if (props.payload.latitude.value && props.payload.longitude.value) {
//     const el = document.createElement('div');
...
//     // Move the map to the saved address
//     map.value.flyTo({ center: [props.payload.longitude.value, props.payload.latitude.value], zoom: 14 });
//   }
// });
...
//     .setLngLat([lng, lat])
//     .setPopup(
//       new mapboxgl.Popup({ offset: 25 }).setText(location.place_name)
//     )
//     .addTo(map.value);
...
// // Define an array to keep track of Mapbox GL markers
// const mapboxMarkers = ref<mapboxgl.Marker[]>([]);
...
//     // Remove previous markers from the Mapbox map
//     mapboxMarkers.value.forEach(marker => marker.remove());
//     mapboxMarkers.value = []; // Clear the marker array
...
//         .setPopup(new mapboxgl.Popup().setText(`Amenity: ${facility.name}`))
//         .addTo(map.value);
      
//       // Add marker to the array for future cleanup
//       mapboxMarkers.value.push(marker);
//     });

//     // Append fetched amenities to `props.payload.neighbouringLandmarks`
//     props.payload.neighbouringLandmarks = [
//       ...(props.payload.neighbouringLandmarks || []),
//       ...amenities.value,
//     ];

//     // Emit the updated list of amenities to the parent component
//     console.log(amenities.value, 'amenities')
//     emit('amenitiesUpdated', amenities.value);
...
//     // Emit the updated list of amenities to the parent component
//     console.log(amenities.value, 'amenities list')
//     // emit('amenitiesUpdated', amenities.value);
