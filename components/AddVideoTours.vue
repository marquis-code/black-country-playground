      ...
      <div v-for="(feature, featureIndex) in currentRoom.features" :key="feature.name" class="mb-6 relative border border-gray-200 rounded-lg">
        ...
        >
          ...
          <div class="absolute inset-0">
            <!-- Spinner for loading -->
            <div v-if="loadingState[featureIndex]" class="absolute inset-0 flex items-center justify-center z-10 bg-gray-100 bg-opacity-70">
          ...
          <button v-if="feature.images.length > 1" @click="prevImage(featureIndex)" class="absolute -left-0 top-1/2 transform -translate-y-1/2 text-gray-700 p-2 z-10">
          ...
          <button v-if="feature.images.length > 1" @click="nextImage(featureIndex)" class="absolute -right-0 top-1/2 transform -translate-y-1/2 text-gray-700 p-2 z-10">
            <svg width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect width="20" height="20" rx="10" fill="black" fill-opacity="0.5"/>
          ...
          <button v-if="feature.images.length" @click="deleteImage(featureIndex)" class="absolute top-2 right-2 text-white p-2  z-10">
        ...
        <div class="flex justify-between items-center bg-[#EBE5E0] p-3">
          <div class="flex items-center">
            <h4 class="text-sm font-medium">{{ feature.name }}</h4>
            <span class="text-sm text-gray-500 ml-2">| {{ feature.images.length }} image{{ feature.images.length !== 1 ? 's' : '' }}</span>
          </div>
          <button @click="triggerFileInput(featureIndex)" class="px-4 py-2 flex gap-x-2 items-center bg-gray-700 text-white text-sm rounded-md hover:bg-gray-600">
            <svg width="10" height="10" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M4 1.3335V6.66683" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M1.33398 4H6.66732" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>              
               Add photo
          </button>
        </div>
        ...
        <input type="file" accept="image/png, image/jpeg" multiple @change="uploadImages($event, featureIndex)" class="hidden" ref="fileInput"/>
      </div>
    </section>
    </div>
  </div>
...
// Watch rooms and persist to local storage when changes occur
...
const deleteImage = (featureIndex: number) => {
  if (currentRoom.value.features[featureIndex].images.length > 0) {
    currentRoom.value.features[featureIndex].images.splice(activeImageIndex.value[featureIndex], 1);

    // Update the carousel index to show the previous or next image
...
const nextImage = (featureIndex: number) => {
  if (activeImageIndex.value[featureIndex] < currentRoom.value.features[featureIndex].images.length - 1) {
    activeImageIndex.value[featureIndex]++;
  }
};
...
// Load rooms from local storage on mount and sync with payload.rooms.value
onMounted(() => {
  const savedRooms = localStorage.getItem('property_rooms');
  if (savedRooms) {
    rooms.value = JSON.parse(savedRooms);
    payload.rooms.value = JSON.parse(savedRooms); // Synchronize with payload.rooms.value
  } else {
    rooms.value = [];
    payload.rooms.value = [];
  }
