    ...
    <div>
      ...
      <div class="flex gap-2 gap-x-6">
        <button 
          ...
          :class="{ 'bg-[#EBE5E0]': activeRoomIndex === index }"
          class="px-4 py-2 border-gray-100 rounded-md text-sm border-[0.5px]"
        >
          {{ room.name }}
        </button>
      </div>
      ...
      <div class="mt-4 space-y-4">
        <div class="flex justify-between items-center">
          <h4 class="font text-[#1D2739]">Is the room furnished?</h4>
          <button 
            :class="{ 'bg-[#292929] text-white': activeRoom.isFurnished, 'bg-gray-200': !activeRoom.isFurnished }" 
            class="px-6 py-2 rounded-md">
            {{ activeRoom.isFurnished ? 'Yes' : 'No' }}
          </button>
        </div>
  
        <div>
          <h4 class="text-[#667185] font-light">Interior area</h4>
          <div class="grid grid-cols-3 gap-3 mt-2">
            <div v-for="(item, index) in activeRoom.interiorItems" :key="index" class="flex bg-white rounded-lg border-[0.5px] items-center gap-2 p-2 py-3 border-gray-100">
              ...
              <span class="text-sm text-[#171717]">{{ item }}</span>
            </div>
            <div class="flex items-center gap-2 p-2 rounded-md">
              <button class="flex items-center font-medium text-sm gap-1 text-[#171717]">
...
<section class="flex justify-between">
    <div>
        <h4 class="font-light text-[#667185]">When is the room available?</h4>
        <p class="text-[#1D2739]">{{ activeRoom.availability }}</p>
      </div>

      <div>
        <h4 class="font-light text-[#667185]">Price</h4>
        <p class="text-[#1D2739]">{{ activeRoom.price }}</p>
      </div>
</section>
  
        <div class="flex justify-between items-center">
          <h4 class="font-light text-[#667185]">{{ activeRoom.roomType }}</h4>
  ...
  // Computed property to get the details of the active room
  const activeRoom = computed(() => {
    return props.rooms[activeRoomIndex.value]
  })
