  ...
  if (target && target.files && target.files.length > 0) {
    for (const file of Array.from(target.files)) {
      try {
        ...
        // Once the upload is successful, handle the response
        if (uploadResponse.value && uploadResponse.value.secure_url) {
          uploadedImages.value.push(uploadResponse.value.secure_url) // Add the secure URL to the array
          images.push(uploadResponse.value.secure_url) // Add the URL to the images array to display in the UI
          ...
          if (props.location === 'common-areas') {
            ...
            if (propertyData) {
              ...
              if (property.commonAreas) {
                ...
                if (commonArea) {
                  commonArea.images = [...(commonArea.images || []), ...uploadedImages.value]
                ...
                if (property.rooms) {
                  ...
                  if (room) {
                    ...
                    if (feature) {
                      ...
                      feature.images = [...(feature.images || []), ...uploadedImages.value]
