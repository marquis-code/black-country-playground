...
const { permissions: permissionsList, loading: creating } = useFetchPermissions();
...
const createRole = () => {
  const payload = {
    role: {
      name: roleTitle.value // Assign the role title here
    },
    permissionIds: selectedPermissions.value // Assuming selectedPermissions is an array of permission IDs
  };
  console.log("Sending payload to backend:", payload);
  // Call your API to send the payload
};
    ...
    <template #header-content>
      <div class="container mx-auto flex items-center justify-between py-3 rounded-lg">
        <div class="flex items-center justify-between gap-x-2 w-full">
          ...
          <div>
            <button :disabled="creating" @click="createRole" class="bg-[#292929] disabled:cursor-not-allowed disabled:opacity-25 text-white px-4 py-3 rounded-lg">{{creating ? 'saving..' : 'Create New Role'}}</button>
          </div>
        </div>
      </div>
    </template>
