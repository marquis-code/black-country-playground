<template>
    <LayoutWithoutSidebar>
      ...
      <main>
        ...
        >
          <aside v-if="!fetchingProperty" class="space-y-4 lg:max-w-xs w-full px-5 lg:px-0">
            <div
              v-for="step in steps"
              :key="step.id"
              class="flex items-center p-4 rounded-lg font-medium"
              :class="{
            ...
            >
              ...
              <svg v-if="activeParentStep !== step.id" width="40" height="40" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span class="font-semibold absolute">{{ step.id }}</span>
              </div>
              <h3
                :class="{
                  'text-white': activeParentStep === step.id,
          ...
          <div v-else class="h-32 lg:max-w-xs w-full animate-pulse bg-slate-300 rounded"></div>
  
          
          <div v-if="!fetchingProperty" class="lg:pl-10 p-4 space-y-6 lg:max-w-screen-7xl w-full">
            <CoreProgressStepper
              v-if="activeParentStep === 1"
              :titles="['Let’s start with the basics', 'Details and Preferences']"
              :totalSteps="2"
              ...
              <template #action-buttons>
                <div class="flex justify-between mt-4">
                  ...
                  <button
                    ...
                    :disabled="basicPropertyInformationStep === 2"
            ...
            :commonAreasList="editPayload.commonAreas"
            :interiorAreas="editPayload.commonAreas"
            :exteriorAreas="editPayload.commonAreas"
            ...
            <AssignProperty  :formData="editPayload" :agents="agentsList" :loading="loadingAgents" v-if="activeParentStep === 4 && finalizeStep === 3">
              <template #action-buttons>
                <div class="flex justify-between mt-4">
                  <button
                    @click="handlePreviousStep"
          ...
          <div v-else class="rounded-md w-full mx-auto ml-6">
            <div class="animate-pulse flex space-x-4">
              <div class="flex-1 space-y-6 py-1 w-full">
                <div class="h-96 w-full bg-slate-300 rounded"></div>
              </div>
            </div>
          </div>
        </div>
  ...
  const { editProperty, initForm, loading: editing, payload: editPayload, fetchingProperty } = useEditProperty()
