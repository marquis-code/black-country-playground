  ...
  <main class="flex flex-col h-screen">
   <main>
    <LayoutWithoutSidebar>
      ...
      <main v-if="!isPreviewMode && !fetchingProperty">
        <div
        ...
        <main v-if="isPreviewMode">
          <div class="container mx-auto py-10">
            ...
            <div>    
              <p class="font-medium text-sm mt-4 pb-3">Property Cover Images</p>    
            ...
            <div class="bg-white rounded-lg shadow-sm mx-auto mt-6">
              ...
              <div v-for="(room, index) in payload.rooms.value" :key="index" class="mb-8 border-b pb-6 text-sm">
                ...
                <h3 class="font-semibold mb-2">
                  {{ room?.name }}
                  <span v-if="room?.isMaster" class="text-sm font-light">(Master Room)</span>
                </h3>
                ...
                <p class="text-gray-600 text-sm font-medium mb-2">Furnished: {{ room?.isFurnished ? 'Yes' : 'No' }}</p>
            ...
            <div class="bg-white rounded- shadow-sm">
              <h2 class="text font-semibold mb-6">Common Area</h2>
            ...
            <div class="bg-white rounded-lg shadow-sm">
              ...
              <form @submit.prevent="submitAnswers">
                <div v-for="(questionObj, index) in payload?.value?.questions" :key="index" class="mb-6">
            ...
            <!-- <div class="bg-white rounded-lg shadow-sm mx-auto mt-6">
              ...
              <div v-for="(room, index) in payload.rooms.value" :key="index" class="mb-8 border-b pb-6">
                ...
                <p v-else-if="room?.availability === 'unavailable'" class="text-red-600 mb-2">Unavailable</p>
                <p v-else class="text-yellow-600 mb-2">Available from: {{ formatDate(room?.availableFrom) }}</p>
                <p class="text-gray-800 text-sm font-medium mb-2">Rent: {{ formatCurrency(room?.rentAmount, room?.currencyCode) }} per {{ room?.rentFrequency }}</p>
                <p class="text-gray-600 text-sm font-medium mb-2">Furnished: {{ room?.isFurnished ? 'Yes' : 'No' }}</p>
            ...
            <!-- <h2 class="text-xl font-semibold">Property Preview</h2>
            <div class="mt-6">
              <p>Name: {{ payload.name.value }}</p>
              <p>Description: {{ payload.description.value }}</p>
            </div>
            <button
              @click="togglePreviewMode(false)"
              class="mt-4 bg-gray-900 text-white px-4 py-2 rounded-md"
            >
              Exit Preview
            </button> -->
          </div>
