<template>
     ...
     <main class="min-h-screen">
        <div class="p-6">
            ...
            <div class="flex space-x-6 mb-4">
              <span class="text-lg font-medium text-green-600 border-b-4 border-green-600 cursor-pointer">All lease documents</span>
              <span class="text-lg font-medium text-gray-500 cursor-pointer">Pending signing</span>
              <span class="text-lg font-medium text-gray-500 cursor-pointer">Signed lease</span>
              <span class="text-lg font-medium text-gray-500 cursor-pointer">Draft</span>
            </div>
        
            <!-- Document Cards -->
            <div class="grid grid-cols-4 gap-6">
              <div v-for="document in documents" :key="document.id" class="border rounded-lg p-4 bg-white shadow-sm">
                <p class="text-sm text-gray-500 mb-2">Lease Agreement</p>
                <p class="text-sm text-gray-600 leading-5">
                  {{ document.content }}
                </p>
                <p class="text-lg font-semibold mt-4">{{ document.title }}</p>
                <p class="text-sm text-gray-600">Tenant: {{ document.tenant || '---' }}</p>
                <p class="text-sm text-gray-400">Last updated: {{ document.updatedAt }}</p>
  ...
  const documents = ref([
    {
      id: 1,
      content: 'This is a legally binding agreement. If not understood, consult an Attorney.',
      title: 'Cummings, Frami and Lynch...',
      tenant: 'Ada Dennis',
      updatedAt: '15th May, 2024',
    },
    {
      id: 2,
      content: 'This is a legally binding agreement. If not understood, consult an Attorney.',
